...
relay_topic = 'IOT_PROJECT_Relay'
global ON
# ON = False
conn = sqlite3.connect('airConditioners.db')
c=conn.cursor()
c.execute("SELECT * FROM airConditionersData WHERE airConditionerNumber=:airConditionerNumber", {'airConditionerNumber': 21})
ON = c.fetchone()[1]
...
class ConnectionDock(QDockWidget):
    ...
    def update_btn_state(self, text):
        ...
        if ON == "open":
            self.ePushtbtn.setStyleSheet("background-color: gray")
            self.mc.publish_to("IOT_PROJECT","close the boiler in the shower ")
            ON= "close"
        else:
            self.ePushtbtn.setStyleSheet("background-color: red")
            self.mc.publish_to("IOT_PROJECT","Open the boiler in the shower ")
            ON= "open"
